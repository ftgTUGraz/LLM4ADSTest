...
## Objective
This repository accompanies the survey paper "[*A Survey on the Application of Large Language Models in Scenario-Based Testing of Automated Driving Systems*](https://arxiv.org/pdf/2505.16587v1)." It compiles and categorizes recent academic and industrial studies on how LLMs are applied across various phases of scenario-based ADS testing, covering key models, prompting strategies, safety considerations, and integration challenges. The repository is continuously updated to reflect the latest advances in the field, with each work summarized in organized tables that include publication details, code resources, and brief descriptions. Contributions are highly encouraged—researchers can suggest new papers through issues or pull requests, [please nominate them for inclusion](https://docs.google.com/forms/d/e/1FAIpQLSdn8aTgKbcia4uTbJNRncjz-DLLJ-8xg9Ds4Z5JPdD-VB2zgQ/viewform).Or you can contact us directly at via [yongqi.zhao@tugraz.at](mailto:yongqi.zhao@tugraz.at) and [dong.bi@tugraz.at](mailto:dong.bi@tugraz.at). We welcome community input to help keep this collection comprehensive and up to date.
...
| [**Automatically Generating Content for Testing Autonomous Vehicles from User Descriptions**](https://ieeexplore.ieee.org/abstract/document/11023959) | 2025 IEEE/ACM 47th International Conference on Software Engineering: New Ideas and Emerging Results (ICSE-NIER) | 2025-04 | – | This paper presents ROADGPT, a system that uses a large language model (ChatGPT) to convert user-provided natural language descriptions into structured JSON road specifications, which are then programmatically transformed into executable 3D road scenarios in the BeamNG.tech simulator. ROADGPT enables automated generation of diverse and realistic testing environments for autonomous vehicles, improving the efficiency and focus of simulation-based testing. |
| [**Why Braking? Scenario Extraction and Reasoning Utilizing LLM**](https://www.arxiv.org/pdf/2507.15874) | arxiv | 2025-07 | – | This paper proposes a framework that uses Large Language Models (LLMs) to understand and classify braking scenarios from real-world driving data. By converting low-level sensor and interaction data into structured natural language, the system enables LLMs to reason about the cause of braking and retrieve scenarios through both category-based and semantic similarity search. The method outperforms rule-based baselines and shows strong generalization to unseen scenarios. |
...
| [**ScenarioFuzz-LLM: Enhancing Diversity in Autonomous Driving Scenario Fuzzing with LLMs**](https://ieeexplore.ieee.org/abstract/document/11033362) |  2025 28th International Conference on Computer Supported Cooperative Work in Design (CSCWD)  | 2025-05 | - |  This work proposes ScenarioFuzz-LLM, a testing framework that leverages large language models (LLMs) to enhance the diversity of autonomous driving scenarios. The LLM performs two key tasks: (1) it evaluates scenario similarity to guide the selection of diverse and critical test seeds, and (2) it generates targeted scenario mutations—modifying vehicle behaviors like position or speed—to overcome diversity bottlenecks during genetic evolution. |
| [**AGENTS-LLM: Augmentative GENeration of Challenging Traffic Scenarios with an Agentic LLM Framework**](https://www.arxiv.org/pdf/2507.13729) |  arxiv  | 2025-07 | [github](https://github.com/mh0797/Agents-LLM/) |  This paper introduces AGENTS-LLM, a framework that uses LLM agents to modify real-world traffic scenarios based on natural language, generating challenging cases for autonomous driving tests. Through reasoning, tool use, and quality checks, it achieves high-quality results even with smaller models, matching human-crafted scenarios in expert and simulation evaluations. |
...
| [**DiaVio: LLM-Empowered Diagnosis of Safety Violations in ADS Simulation Testing**](https://dl.acm.org/doi/pdf/10.1145/3650212.3652135) | ISSTA 2024: Proceedings of the 33rd ACM SIGSOFT International Symposium on Software Testing and Analysis | 2024-09 | - | This paper proposes DiaVio, a large language model–powered framework that automatically diagnoses safety violations in autonomous driving simulation testing by determining liability and classifying crash types using a custom crash description language. It significantly reduces manual effort and improves diagnosis efficiency and accuracy. |
